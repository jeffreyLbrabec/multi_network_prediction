...
glia_trial_results <- read_rds(here("results/glia_linear_results/glia_trial_linear_results.rds"))

# Aggregate Model Performance Across Hyper-Parameters ---------------------

glia_fold_res <- map(glia_trial_results, format_folds, learner = "svm_linear")
...
dir.create(file.path(here("results"), paste("glia", "linear", "final", "predictions", sep = "_")))

best_glia_mod_list <- list(splits = best_glia_models$splits,
                            models = best_glia_models$models,
                            fold_id = best_glia_models$fold_id)

final_res_dir <- here("results/glia_linear_final_predictions")
...
write_rds(final_glia_auc, here("results/glia_linear_final_predictions/final_glia_linear_auc.rds"))
